...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const initSystem = async () => {
        ...
        // Safety timeout: If db.init() hangs (e.g. Supabase network issue), don't stay loading forever
        const timeout = setTimeout(() => {
          if (isLoading) {
            console.warn('[SYSTEM] db.init() taking too long. Proceeding to menu...');
            setIsLoading(false);
          }
        }, 10000); // 10s timeout
        ...
        // Fetch Admin Info from Backend with Retry
        const fetchAdminInfo = (retries = 3) => {
          fetch('/api/admin')
            .then(res => {
            ...
            .catch(err => {
              ...
              if (retries > 0) {
                console.log(`Retrying admin info fetch... (${retries} left)`);
                setTimeout(() => fetchAdminInfo(retries - 1), 2000);
              }
            });
        };
        fetchAdminInfo();
