...
// ================= API ROUTES =================
app.get('/api/health', (req, res) => res.json({ status: 'ok', time: new Date().toISOString(), platform: process.platform }));
...
async function processAIResponse(userMessage, contactName, contactPhone, media = null) {
    ...
    while (retryCount < maxRetries) {
        try {
            const ai = new GoogleGenAI(CONFIG.API_KEY);
    ...
    server.listen(port, "0.0.0.0", async () => {
        ...
        try {
            if (fs.existsSync("./.wwebjs_auth")) {
                console.log("[SYSTEM] Auth session found, scheduling auto-start...");
                setTimeout(startWhatsApp, 5000); // Increased delay
            }
        } catch (e) {
            console.error("[SYSTEM] Auto-start check failed:", e.message);
        }
